...
function receiveSMSMessage(wit_client, twilio_client, senderId, message, receiveMessage) {
  ...
  .then((data) => {
    ...
    generateResponseAndNewContext(context, data, senderId, receiveMessage, function(err, context, response, continuing) {
			if (err) cb(err, false);
...
function getResponse(wit_client, senderId, message, cb) {
	console.log("RECEIVED FACEBOOK MESSAGE!", senderId, message);
  ...
  .then((data) => {
    ...
    generateResponseAndNewContext(context, data, senderId, getResponse, function(err, context, response, continuing) {
			if (err) cb(err, false);
  ...
  .catch(console.error);
	...
	//cb(null, "I received your message: " + message);
